...
def main(String platforms, Map options) {
	...
	timestamps {
	    ...
	    if (PRODUCTION) {
		options['django_url'] = "https://nab.cis.luxoft.com/jenkins_post_form/"
		options['plugin_storage'] = "https://nab.cis.luxoft.com/media/plugins/"
		options['cis_tools'] = "RenderSceneJob"
		options['jenkins_job'] = "RenderJob_NAB"
	    } else {
		...
		for (i = 0; i < platformCount; i++) {
			...
			node("${osName} && NabRenderService && ${renderDevice}")
			{
